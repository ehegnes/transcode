...
static void spudec_process_data(spudec_handle_t *this)
{
  ...
  if (this->image_size < this->stride * this->height) {
    ...
    this->image = malloc(this->stride * this->height);
    if (this->image) {
...
static void spudec_process_control(spudec_handle_t *this, int pts100)
{
  ...
  while (start_off != next_off) {
	...
	*/
	/* data are here ignored because we don't need them for subtitle ripping*/
	off+=2;
	break;
      case 0x05:
	/* DISPLAY AREA 
...
static void spudec_writeout_ppm(spudec_handle_t* this)
{
  ...
  */
  ...
  ppm_writeppm(out_file, ppm_image, width,
	       height, 255, 0 /*forceplane*/);

  fclose(out_file);
  ppm_freearray(ppm_image, this->height);  
#else
  fprintf(stderr,"PPM not supported. Please recompile with -D_HAVE_LIB_PPM_\n");
  exit(1);
#endif  
}


static void spudec_writeout_tag(spudec_handle_t *this, int pts100)
{
...
static void spudec_decode(spudec_handle_t *this, int pts100)
{
  ...
  spudec_writeout_tag(this, pts100);
...
void spudec_assemble(spudec_handle_t *this, unsigned char *packet, int len, int pts100)
{
  ...
  /* check if we have a complete packet 
     (unfortunatelly packet_size is bad
     for some disks)
  */
...
spudec_handle_t * spudec_new(int color[4], char* ppm_base_name, 
			     char* tag_base_name, output_formats format, int crop)
{
