...
unsigned int     firstsync)
{
  ...
  *(index++) = type;
  ...
  else
  {
    ...
    if (svcd_offset)
    {
      l = fread(&data_buffer[data_bytes], 1, svcd_offset, inputstream);
      if (ferror(inputstream))
      {
	 DisplayError("Error reading from an input file.");
	 return FALSE;
      }
      data_bytes += l;
      if (l != svcd_offset)
    ...
    l = fread(&data_buffer[data_bytes], 1, i - data_bytes, inputstream);
    if (ferror(inputstream))
    {
      DisplayError("Error reading from an input file.");
	  return FALSE;
