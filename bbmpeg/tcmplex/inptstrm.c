...
unsigned int which_streams)
{
  ...
  if (which_streams & STREAMS_AUDIO)
  {
    ...
    retval=0;
    while(retval==0x0) {//avoid zero padding
  ...
  *startup_delay = 2 * MAX_FFFFFFFF;
	  ...
	  picture_type = retval;
          ...
          if (frame_count > f_rate)
          {
            if (tmp_offset - mb_offset > max_bits_in_sec)
              max_bits_in_sec = (long unsigned int)(tmp_offset - mb_offset);
  ...
  if (!AbortMPEG)
  {
    ...
    if (secs_per_frame >0.)
    {
      ...
      video_info->comp_avg_bit_rate = 
        (unsigned int)(ceil ((double)(video_info->stream_length) /
...
Audio_struc *audio_info)
{
  ...
  {                                   /* MPEG */
    ...
    if (audio_info->bit_rate == 0)
      ...
      {
	     sprintf(tmpStr, "    Bit rate : %lu bytes/sec (%lu kbit/sec)",
		    bitrate*125, bitrate);
        DisplayInfo(tmpStr);
      }
...
unsigned int length)
{
  ...
  unsigned int i;
  unsigned int retval=0, audio_count;
  int percent, oldPercent;
  int exiterror;
  char tmpStr[132];
  bitstream bs;
  ...
  if (retval==AUDIO_SYNCWORD)
  {
    ...
    framesize = (unsigned int)(bitrate_index[3-audio_info->layer][audio_info->bit_rate] / frequency[audio_info->frequency] * slots [3-audio_info->layer]);

    //ThOe infos:
    fprintf(stderr, "\nThOe: layer=%d bit_rate=%d freq=%f slots=%d\n", 3-audio_info->layer, bitrate_index[3-audio_info->layer][audio_info->bit_rate], frequency[audio_info->frequency], slots [3-audio_info->layer]);
    //--
