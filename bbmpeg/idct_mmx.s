...
;;-- defined in C-module for convenience --
extern bb_r_inv_row
extern bb_r_inv_col
extern bb_r_inv_corr

extern bb_tab_i_01234567
extern bb_qwTemp
...
global bb_idct_mmx_rows
global bb_idct_mmx_cols
;;
;; bb_idct_mmx_rows( short *blk )	;; transform all 8 rows of 8x8 iDCT block
;;
...
bb_idct_mmx_rows:
	...
	push edi
        ...
        mov INP, [ebp+8];		;; row 0
	mov edi, 0x00;	                ;; x = 0
	lea TABLE, [bb_tab_i_01234567];    ;; row 0

	lea OUTP, [bb_qwTemp];
	lea round_inv_row, [bb_r_inv_row]
...
lpa:
	...
	movq mm0,  [OUTP + ROW_STRIDE * 0 ]

	movq mm1,  [OUTP + ROW_STRIDE * 1 ]
	movq mm4, mm0;	;; mm4 = copy of row1[A B C D]
	
	movq mm2, [OUTP + ROW_STRIDE * 2 ]
	punpcklwd mm0, mm1; ;; mm0 = [ 0 4 1 5]
	
	movq mm3, [OUTP + ROW_STRIDE * 3]
	...
	movq mm1,  [OUTP + ROW_STRIDE*5 + 8]

	movq  [ INP +ROW_STRIDE * 3], mm3; ;; store row 4
	punpcklwd mm0, mm1; ;; mm0 = [ 0 4 1 5]

	;; 2) transpose lower-right quadrant
	movq mm2,  [OUTP + ROW_STRIDE*6 + 8]
	punpckhwd mm4, mm1; ;; mm4 = [ 2 6 3 7]
	
	movq mm3,  [OUTP + ROW_STRIDE*7 + 8]
	movq mm6, mm2;
	...
	;; 2) transpose lower-right quadrant
	movq mm2,  [OUTP + ROW_STRIDE*2 + 8]
	punpckhwd mm4, mm1; ;; mm4 = [ 2 6 3 7]

	movq mm3,  [OUTP + ROW_STRIDE*3 + 8]
	movq mm6, mm2;
...
;;
;; bb_idct_mmx_cols( short *blk )	;; transform all 8 cols of 8x8 iDCT block
;;
bb_idct_mmx_cols:
	push ebp			;; save stack pointer
	mov ebp, esp		        ;; link
	...
	lea TABLE,  [bb_tab_i_01234567]    ;; row 0
	lea OUTP,  [bb_qwTemp];
	lea round_inv_col,  [bb_r_inv_col]
