...
int tc_filter(frame_list_t *ptr_, char *options)
{
  ...
  if(ptr->tag & TC_FILTER_INIT) {
	...
	{
		mfd->saveFrame = (int *) malloc (width*height*sizeof(int));
	}
  ...
  if(ptr->tag & TC_FILTER_GET_CONFIG) {
      ...
      snprintf (buf, sizeof(buf), "%d", mfd->colordiff);
