...
void RateCtlInit(RateControl* pCtl, double quant, double target_rate, 
...
int RateCtlGetQ(RateControl* pCtl, double MAD)
{
	double quant;
...
//	if (MAD < 5.) 
//		quant = pCtl->iMinQuantizer + (quant - pCtl->iMinQuantizer) * MAD / 5.;

	return (int)(quant + 0.5);
}

void RateCtlUpdate(RateControl* pCtl, int current_frame)
{
	...
	median_quant = pCtl->iMinQuantizer + (pCtl->iMaxQuantizer - pCtl->iMinQuantizer) / 2;

	/* reduce quantizer when the reaction rate is low */
	if (rate < current_target) pCtl->quant *= 
		(1 - pCtl->reaction_delta * ((current_target - rate) / current_target / 0.20) );
	if (pCtl->quant < pCtl->iMinQuantizer) pCtl->quant = pCtl->iMinQuantizer;
	...
	if (rate > current_target) {
		...
		else if (rate > current_target * 1.20) pCtl->quant *=
			(1 + pCtl->reaction_delta);
		else pCtl->quant *= 
			(1 + pCtl->reaction_delta * ((rate - current_target) / current_target / 0.20) );
	}
	if (pCtl->quant > pCtl->iMaxQuantizer) pCtl->quant = pCtl->iMaxQuantizer;
