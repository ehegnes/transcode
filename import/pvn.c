...
long calcPVNPageSize(PVNParam p)
{
  ...
  {
    fprintf(stderr, "Unknown PVN format type, only a, f and d are acceptable");
    exit(1);
  }
...
{
  printf("Magic: %s, Width: %d, Height: %d, Depth: %d, Framerate: %d Maxval: %f\n", 
...
int readPVNHeader(FILE *fp, PVNParam *p)
{
  ...
  p->depth=-1; p->framerate=-1; /* clear these vars */
  ...
  while(!done)
  {
    ...
    if (tmpMagic[0] == 0)
    {
      sscanf(line, "%s %d %d %d %lf %d", tmpMagic, &(p->width), &(p->height), &(p->depth), &(p->maxcolour), &(p->framerate));
    ...
    /* read remaining variables as they're available */
      ...
      sscanf(line, "%d %d %d %lf %d", &(p->width), &(p->height), &(p->depth), &(p->maxcolour), &(p->framerate));
    else if (p->height == -1)
      sscanf(line, "%d %d %lf %d", &(p->height), &(p->depth), &(p->maxcolour), &(p->framerate));
    else if (p->depth == -1)
      sscanf(line, "%d %lf %d", &(p->depth), &(p->maxcolour), &(p->framerate));
    else if (p->maxcolour == -1)
      sscanf(line, "%lf %d", &(p->maxcolour), &(p->framerate));
    else if (p->framerate == -1)
      sscanf(line, "%d", &(p->framerate));

    if (p->framerate != -1) /* we're done */
    {
      ...
      {
        fprintf(stderr, "Bitmap PV4x files cannot be in float or double format!\n");
        return(INVALID);         
      }
      ...
      else if ((p->magic[3] == 'a') && (((int)p->maxcolour % 8 != 0) || (p->maxcolour > 32) || (p->maxcolour==0)))
      {
  ...
  if ((p->magic[0] == 'P') && (p->magic[1] == 'V') && ((p->magic[3] == 'a') || (p->magic[3]=='f') || (p->magic[3]=='d')))
  {
    switch(p->magic[2])
    {
      case '1':
...
//  unsigned int i;
  ...
  if ((format != FORMAT_FLOAT) && (format!= FORMAT_DOUBLE) && (format != FORMAT_INT))
  {
    fprintf(stderr, "Invalid output format!\n");
    return(ERROR);
  }

  if ((format == FORMAT_INT) && ((maxcolour > 32) || (maxcolour <= 0) || ((int)maxcolour % 8 != 0)))
  {
    fprintf(stderr, "Invalid maxcolour value, must be multiple of 8, and a max of 32!\n");
    return(ERROR);
  }

  if ((format != FORMAT_INT) && (maxcolour <= 0))
  {
  ...
  if ((inParams.magic[2] == '4') && ((format != inFormat) || (inParams.maxcolour != maxcolour)))
  {
    fprintf(stderr, "Cannot change format of a PV4a file!\n");
    return(ERROR);
  }
...
//  for(i=0; i < inParams.depth; i++)
  ...
  {
    if((inParams.magic[2] == '4') || ((inFormat == format) && (inParams.maxcolour == outParams.maxcolour)))
    {
#ifdef DEBUG
