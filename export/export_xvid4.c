...
#define CFGPREFIX MOD_NAME
	/* Read the values */
	module_read_config("features",  CFGPREFIX, "xvid4", features_config);
	module_read_config("quantizer", CFGPREFIX, "xvid4", quant_config);
	module_read_config("cbr",       CFGPREFIX, "xvid4", onepass_config);
	module_read_config("vbr",       CFGPREFIX, "xvid4", pass2_config);
#undef CFGPREFIX
...
#define CFGPREFIX "["MOD_NAME"] "
	...
	if(verbose_flag & TC_DEBUG) {
		fprintf(stderr,
			"\n\n[%s] Feature settings\n[%s] ----------------------------------------\n",
			MOD_NAME, MOD_NAME);
		module_print_config(CFGPREFIX, features_config);

		fprintf(stderr,
			"\n[%s] Quantizer settings\n[%s] ----------------------------------------\n",
			MOD_NAME, MOD_NAME);
		module_print_config(CFGPREFIX, quant_config);

		fprintf(stderr,
			"\n[%s] CBR settings\n[%s] ----------------------------------------\n",
			MOD_NAME, MOD_NAME);
		module_print_config(CFGPREFIX, onepass_config);

		fprintf(stderr,
			"\n[%s] VBR settings\n[%s] ----------------------------------------\n",
			MOD_NAME, MOD_NAME);
		module_print_config(CFGPREFIX, pass2_config);
	}
#undef CFGPREFIX
