...
MOD_init
{
    ...
    if(param->flag == TC_VIDEO) {
      if(verbose & TC_DEBUG)
          fprintf(stderr, "[%s] max AVI-file size limit = %lu bytes\n",
...
MOD_encode
{
  ...
  if(param->flag == TC_VIDEO) {
    ...
    if (r == LZO_E_OK) {
      if(verbose & TC_DEBUG) fprintf(stderr, "compressed %lu bytes into %lu bytes\n",
				    (long) param->size, (long) out_len);
    } else {
      /* this should NEVER happen */
      fprintf(stderr, "[%s] internal error - compression failed: %d\n", MOD_NAME, r);
      return(TC_EXPORT_ERROR); 
    }
    ...
    if (out_len >= param->size)  {
      if(verbose & TC_DEBUG)
          fprintf(stderr, "[%s] block contains incompressible data\n", MOD_NAME);
  ...
  if(param->flag == TC_AUDIO)
      return(audio_encode(param->buffer, param->size, avifile2));
