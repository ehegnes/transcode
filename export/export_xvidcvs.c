...
#define MOD_VERSION "v0.3.3 (2002-04-17)"
#if API_VERSION == ((2 << 16) | (0))
#define MOD_CODEC   "(video) Xvid API 2.0 (CVS) | (audio) MPEG/AC3/PCM"
#else
#define MOD_CODEC   "(video) Xvid API 1.0 (CVS) | (audio) MPEG/AC3/PCM"
#endif
...
MOD_init
{
  ...
  if(param->flag == TC_VIDEO) 
  {
	...
	if ( (VbrMode==3) )
		xparam.rc_bitrate = 0;		// disable rate-control
	else 
	  	xparam.rc_bitrate = vob->divxbitrate*1000;
...
#else
	xparam.rc_averaging_period = vob->rc_reaction_period; 
	xparam.rc_reaction_delay_factor = vob->rc_reaction_ratio; 
...
/* so the rest is TC_VIDEO only */
	...
	//enable field based encoding if requested (ThOe 2002-06-08)
	if(encode_fields) xframe.general |= XVID_INTERLACING;
